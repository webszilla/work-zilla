...
def public_plans(request):
    ...
    for plan in plans:
        if normalized_slug == "storage":
            if hasattr(plan, "monthly_price_inr"):
                ...
                response_plans.append({
                    ...
                    "name": plan.name,
                    "actual_price_inr_month": float(plan.actual_monthly_price or 0),
                    "actual_price_usdt_month": float(plan.actual_usd_monthly_price or 0),
                    "actual_price_inr_year": float(plan.actual_yearly_price or 0),
                    "actual_price_usdt_year": float(plan.actual_usd_yearly_price or 0),
        ...
        response_plans.append({
            ...
            "code": slugify(plan.name),
            "name": plan.name,
            "actual_price_inr_month": plan.actual_monthly_price or 0,
            "actual_price_usdt_month": plan.actual_usd_monthly_price or 0,
            "actual_price_inr_year": plan.actual_yearly_price or 0,
            "actual_price_usdt_year": plan.actual_usd_yearly_price or 0,
