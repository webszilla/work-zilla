(function () {
  ...
  function buildUI(mode) {
    ...
    wrapper.innerHTML = [
      ...
      '      <div class="wz-ai-chatbot__status">Online</div>',
      '    </div>',
      ...
      '    <button type="button" class="wz-ai-chatbot__attach" aria-label="Attach file"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 7.5v8.25a5 5 0 0 0 10 0V6.5a3.5 3.5 0 0 0-7 0v8.75a2 2 0 0 0 4 0V8.5" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg></button>',
      '    <input type=\"file\" class=\"wz-ai-chatbot__file\" accept=\".png,.jpg,.jpeg,.gif,.webp,.pdf,.doc,.docx,.xls,.xlsx,.txt,.csv\" />',
      ...
      '    <div class="wz-ai-chatbot__brand"><a href="https://getworkzilla.com" target="_blank" rel="nofollow">AI Chatbot Powered by Work Zilla</a></div>',
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      ...
      ".wz-ai-chatbot__status{font-size:11px;color:rgba(226,232,240,0.7);}",
      ...
      ".wz-ai-chatbot__attachment{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;margin-top:6px;border-radius:10px;background:rgba(148,163,184,0.18);color:inherit;text-decoration:none;font-size:12px;}",
      ".wz-ai-chatbot__attachment-image{max-width:220px;border-radius:8px;display:block;}",
      ...
      ".wz-ai-chatbot__emoji{background:transparent;border:1px solid var(--wz-primary);color:var(--wz-primary);border-radius:999px;padding:6px 10px;font-size:12px;}",
      ".wz-ai-chatbot__attach{background:transparent;border:1px solid var(--wz-primary);color:var(--wz-primary);border-radius:999px;padding:6px 10px;font-size:12px;display:inline-flex;align-items:center;justify-content:center;}",
      ".wz-ai-chatbot__attach svg{width:16px;height:16px;}",
      ".wz-ai-chatbot__file{display:none;}",
  ...
  function normalizeApiBase(value, el) {
    if (!value) {
      var origin = getScriptOrigin(el) || window.location.origin;
      return origin + "/api/ai-chatbot";
    }
    if (/^https?:\/\//i.test(value)) {
      return value.replace(/\/$/, "");
    }
    if (value.charAt(0) === "/") {
      var scriptOrigin = getScriptOrigin(el) || window.location.origin;
      return scriptOrigin + value;
    }
    return value;
  }

  var apiBase = normalizeApiBase(script.getAttribute("data-api-base"), script);
  ...
  var inputEl = ui.querySelector(".wz-ai-chatbot__input input[type='text']");
  var sendBtn = ui.querySelector(".wz-ai-chatbot__input button:last-of-type");
  var emojiBtn = ui.querySelector(".wz-ai-chatbot__emoji");
  var attachBtn = ui.querySelector(".wz-ai-chatbot__attach");
  var attachInput = ui.querySelector(".wz-ai-chatbot__file");
  ...
  var allowVisitorAttachments = false;
  ...
  function updateAttachmentUI() {
    ...
    attachBtn.style.display = allowVisitorAttachments ? "inline-flex" : "none";
  }
  ...
  function renderMessages(messages) {
    ...
    messages.forEach(function (msg) {
      ...
      if (msg.attachment_url) {
        if (msg.attachment_type && msg.attachment_type.indexOf("image/") === 0) {
          var link = document.createElement("a");
          link.href = msg.attachment_url;
          link.target = "_blank";
          link.rel = "noreferrer";
          link.className = "wz-ai-chatbot__attachment";
          var img = document.createElement("img");
          img.src = msg.attachment_url;
          img.alt = msg.attachment_name || "Attachment";
          img.className = "wz-ai-chatbot__attachment-image";
          ...
          fileLink.className = "wz-ai-chatbot__attachment";
          fileLink.textContent = msg.attachment_name || "Download attachment";
  ...
  function fetchConfig() {
    return fetch(apiBase + "/widget/config?key=" + encodeURIComponent(widgetKey))
      ...
      .then(function (data) {
        ...
        if (statusEl) {
          statusEl.textContent = "Online · Typically replies in a few minutes";
        }
        if (data && data.theme) {
          applyTheme(data.theme);
        }
        allowVisitorAttachments = !!(data && data.allow_visitor_attachments);
        updateAttachmentUI();
        return data;
      });
  }
  ...
  function fetchThread() {
    ...
    return fetch(url)
      ...
      .then(function (data) {
        ...
        if (typeof data.allow_visitor_attachments !== "undefined") {
          allowVisitorAttachments = !!data.allow_visitor_attachments;
          updateAttachmentUI();
        }
  ...
  function sendAttachment(file) {
    ...
    formData.append("key", widgetKey);
    formData.append("visitor_id", visitorId);
    formData.append("category", currentCategory);
    formData.append("source", source);
    formData.append("name", visitorProfile.name || "");
    formData.append("email", visitorProfile.email || "");
    formData.append("phone", visitorProfile.phone || "");
    formData.append("file", file);
    return fetch(apiBase + "/widget/attachment", {
      method: "POST",
      body: formData
    })
      .then(function (response) {
        if (!response.ok) {
          return response.json()
            .then(function (data) {
              appendLocalBotMessage(data && data.message ? data.message : "Unable to upload file right now.");
              return null;
            })
            .catch(function () {
              appendLocalBotMessage("Unable to upload file right now.");
              return null;
            });
        }
        return response.json();
