...
class BusinessAutopilotSiteAdminQuickEstimateTests(TestCase):
    ...
    def test_quick_estimate_payment_update_keeps_online_proof_for_partial_payment(self):
        created = self.client.post(
            "/api/business-autopilot/site-admin/chat",
            data={
                "message": "9092833701\nGuru\nBusiness Card 500 nos Rs.1050",
        ...
        response = self.client.post(
            ...
            data=json.dumps({
                "__action": "PATCH",
                "action": "payment",
                "payment_status": "non_completed",
                "payment_mode": "online",
                "payment_proof_entries": [{"image": image_one, "paid_date": "2026-07-01"}],
            }),
            content_type="application/json",
        )
        ...
        self.assertEqual(response.status_code, 200)
        estimate = QuickEstimate.objects.get(organization=self.org, id=estimate_id)
        self.assertEqual(estimate.payment_status, QuickEstimate.PROGRESS_NON_COMPLETED)
        self.assertEqual(estimate.payment_mode, "online")
        self.assertEqual(json.loads(estimate.payment_proof_image), [{"image": image_one, "paid_date": "2026-07-01"}])
        detail = self.client.get(f"/api/business-autopilot/quick-estimates/{estimate_id}/")
        self.assertEqual(detail.status_code, 200)
        quick_estimate = detail.json()["quick_estimate"]
        self.assertEqual(quick_estimate["payment_status"], "non_completed")
        self.assertEqual(quick_estimate["payment_mode"], "online")
        self.assertTrue(quick_estimate["payment_partial_paid"])
