...
def _get_org_from_company_key(request):
    ...
    if employee_id:
        ...
        if employee and employee.org:
            ...
            if not has_active_subscription_for_org(employee.org, "monitor"):
    ...
    if not has_active_subscription_for_org(org, "monitor"):
        return None, Response({"error": "subscription_required"}, status=403)
    return org, None
...
def monitor_stop_event(request):
    ...
    try:
        ...
        return Response({"message": "Monitor stop recorded"})
