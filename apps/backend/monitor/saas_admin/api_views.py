...
from core.subscription_utils import is_subscription_active, revert_transfer_subscription
...
def _render_invoice_pdf_bytes(transfer, billing_profile):
    ...
    doc.drawString(left, y, seller.get("name", "Work Zilla Monitor"))
    ...
    if addon_qty or is_addon_only:
        ...
        desc_width = 88 * mm
        doc.setFont("Helvetica-Bold", 9)
        doc.drawString(left, row_y, "Monitor")
        doc.setFont("Helvetica", 9)
...
def _get_product_org_ids(slug):
    ...
    catalog_product = CatalogProduct.objects.filter(slug=slug).first()
    if catalog_product:
    ...
    if slug == "monitor":
        core_subs = Subscription.objects.filter(models.Q(plan__product__slug=slug) | models.Q(plan__product__isnull=True))
        org_ids.update(core_subs.values_list("organization_id", flat=True))
        history = SubscriptionHistory.objects.filter(models.Q(plan__product__slug=slug) | models.Q(plan__product__isnull=True))
        org_ids.update(history.values_list("organization_id", flat=True))
...
def product_detail(request, slug):
    ...
    if slug in ("storage", "online-storage"):
        ...
        for sub in subs:
            ...
            normalized = "trial" if status == "trialing" else ("active" if status == "active" else "inactive")
            ...
            org_payload.append({
                ...
                "owner_email": owner.email if owner else "",
                "status": status,
                "enabled_at": _format_date(sub.created_at),
            })
        plans_payload = [
            {
                "id": plan.id,
    ...
    for org in orgs:
        ...
        if not status and slug == "monitor":
    ...
    pending_approvals = PendingTransfer.objects.filter(
        organization_id__in=org_ids,
        status="pending",
    ).count()
    ...
    if product.slug == "monitor":
        ...
        plan_rows = Plan.objects.filter(product__slug=product.slug)
    plans_payload = [
        {
            "id": plan.id,
            "name": plan.name,
...
def product_organizations(request, slug):
    ...
    if slug in ("storage", "online-storage"):
        ...
        core_subs = (
            Subscription.objects
            .filter(organization_id__in=list(org_ids), plan__product__slug=slug)
            .select_related("plan")
            .order_by("organization_id", "-start_date")
        )
        ...
        for org in orgs:
            ...
            payload.append({
                ...
                "owner_name": owner_name,
                "owner_email": owner.email if owner else "",
                "subscription": _serialize_storage_subscription(sub_by_org.get(org.id)),
            })
        plans_payload = [
            {"id": plan.id, "name": plan.name}
    ...
    payload = [_serialize_org(org, sub_by_org.get(org.id)) for org in orgs]
    plans_payload = [
        {"id": plan.id, "name": plan.name}
        for plan in Plan.objects.all().order_by("name")
    ]
    ...
    if slug == "ai-chatbot":
        org_transfers = (
            ...
            .filter(
                organization_id__in=list(org_ids),
                status="pending",
                plan__product__slug="ai-chatbot",
            )
            .select_related("organization", "plan")
            .order_by("-created_at")
        )
...
def product_pending_transfer_action(request, slug, transfer_id, action):
    ...
    if slug in ("storage", "online-storage"):
        if not transfer.plan or transfer.plan.product_id != product.id:
            return HttpResponseForbidden("Access denied.")
    elif slug == "ai-chatbot":
        if not transfer.plan or transfer.plan.product_id != product.id:
            return HttpResponseForbidden("Access denied.")
...
def product_billing_history(request, slug):
    ...
    plan_product = CatalogProduct.objects.filter(slug=slug).first()
    if slug == "monitor":
        if plan_product:
        ...
        plan_filter = models.Q(plan__product=plan_product) if plan_product else models.Q(plan__product__slug=slug)
